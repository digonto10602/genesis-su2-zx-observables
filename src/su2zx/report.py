...
def strategy_table(frame: pd.DataFrame) -> str:
    medians = frame.groupby("strategy")[
        ["native_2q_count", "native_2q_depth", "routing_penalty_ratio"]
    ].median()
    lines = [
    ...
    for strategy, row in medians.iterrows():
        lines.append(
            f"| {strategy} | {row.native_2q_count:.1f} | {row.native_2q_depth:.1f} | {row.routing_penalty_ratio:.3f} |"
        )
    return "\n".join(lines)
...
def main() -> None:
    ...
    p_error = physics.get("trotter_convergence_fit", {}).get(
    ...
    seed_span = seeds.groupby(["target_topology", "strategy"]).native_2q_count.agg(
        lambda x: int(x.max() - x.min())
    )
    test_log = output / "logs" / "pytest.log"
    test_evidence = (
        test_log.read_text(encoding="utf-8")[-1200:] if test_log.exists() else "not run"
    )
    test_line = next(
        (line.strip() for line in reversed(test_evidence.splitlines()) if "passed" in line),
        "not yet recorded",
    )
